...
void    HttpResponseUtils::initStatusMap(void) {
    if(HttpResponseUtils::codes.empty()) {
        statusMap[200] = "OK";
...
std::string HttpResponseUtils::getStatus(HttpStatusCode responseCode) {
    std::map<HttpStatusCode, std::string>::iterator it = HttpResponseUtils::codes.find(responseCode);
    if (it != HttpResponseUtils::codes.end()) {
        return (it->second);
    }
    return (NULL);
}
...
std::string HttpResponseUtils::errorBody(HttpStatusCode responseCode) {
    ...
    <html>
    <head>
        <title>Error " << responseCode <<"</title>
        <style>
    ...
    <div class=\"error-middle\">
        <h1>Error " << responseCode << " - " << getStatus(responseCode) << "</h1>
    </div>
    </body>
    </html>";
    return (response.str());
